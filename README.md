<div align="center">
  <img src="./png/logo.png" alt="月月岛科技 Logo" width="180" />
</div>

<div align="left">
  <h1>月月岛科技</h1>
  <p><strong>开源技术社区 / 个人兴趣 / 创作爱好</strong></p>
  <p>
    它不是一间现实中的科技公司，
    <br />
    而是一座由热爱、开源与想象力共同搭起的小岛。
  </p>
  <p>
    <strong>文档信息</strong>
    <br />
    这里会持续记录项目设定、创作灵感、内容整理与岛屿世界观。
  </p>
  <p>
    <strong>详细信息引用</strong>
    <br />
    引用自 <a href="./clutter/Details.md"><strong>《杂物 · 月月岛科技详细说明》</strong></a>
  </p>
  <blockquote>
    <p>
      月月岛科技是一个以开源技术、个人兴趣和创作爱好为核心的内容集合。
      它更接近一种个人化的技术社区设定、作品归档方式与项目世界观，而不是现实中的实体化科技公司。
    </p>
  </blockquote>
  <p>
    <a href="#关于这座岛"><strong>阅读岛屿介绍</strong></a>
    ·
    <a href="#人物信息"><strong>查看人物信息</strong></a>
  </p>
</div>

<br />

<table>
  <tr>
    <td width="320" valign="top" align="left">
      <img src="./png/yueyue.png" alt="月月岛科技吉祥物" width="280" />
      <br />
      <sub>月月岛科技吉祥物 · 个人原创设计</sub>
    </td>
    <td valign="top" align="left">
      <h3 id="关于这座岛">关于这座岛</h3>
      <p>
        月月岛科技是一个围绕开源技术、个人兴趣与创作爱好而形成的项目集合。
        这里记录灵感、承载作品，也用一套温柔又带点幻想感的“虚拟科技”设定，
        把分散的内容慢慢连成完整世界。
      </p>
      <p>
        所有项目都出于兴趣而生，所有表达都以分享、记录、连接为核心。
        它更像是一个持续生长的开源角落，而不是一个实体化商业机构。
      </p>
      <h3 id="人物信息">人物信息</h3>
      <p>
        <strong>月月</strong> 是这座岛的形象代表与陪伴角色。
        她让技术、创作和故事感自然地连接在一起，也让整个视觉设定更温柔、更有记忆点。
      </p>
    </td>
  </tr>
</table>

<div align="left">
  <sub>开源是连接，兴趣是起点，创作让这座岛慢慢发光。</sub>
</div>
